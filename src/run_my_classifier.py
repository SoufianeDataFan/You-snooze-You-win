...
def classify_record(index):
    ...
    # ------------------------------------------------------------------
    # Return a vector of per-sample predictions
    # ------------------------------------------------------------------
...
def run_classifier(header_file, signal_file, classifier_pickle):
    ...
    signal_names, Fs, n_samples = 
    .import_signal_names(header_file)

    # Get this subject's data as a dataframe
    this_data = get_subject_data_test(signal_file, signal_names)

    # For the baseline, let's only look at how SaO2 might predict arousals
    SaO2 = this_data.get(['SaO2']).values

    # Format the data to fit model input shape
    step        = Fs * 60
    window_size = Fs * 60
...
if __name__ == '__main__':
    ...
    for index in range(len(df)) :
        ...
        results = classify_record(record_name)
